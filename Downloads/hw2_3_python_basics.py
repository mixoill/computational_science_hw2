...
Original file is located at
    https://colab.research.google.com/drive/1Y2Q1XbjZFGeOGzKckj3-g_bF-dtA7rKp
...
print(extract_and_apply([1, 2, 3, 4], lambda x: x % 2 == 0, lambda x: x))
...
def concatenate(seqs):
    result = [item for sublist in seqs for item in sublist]
    return result
...
def slices(seq):
  ...
  for j in range(len(seq)):
    for i in range(1, len(seq)-j+1):
      result.append(seq[j:j+i])
  return result
...
def normalize(text):
    ...
    for item in range(len(text)-1):
      if text[item] == ' ' and text[item+1] == ' ':
        continue
      else:
        new_text.append(text[item])
...
df["Work Time"] = df["SubmitTime"] - df["AcceptTime"]
def get_work_time(df):
    return df.groupby("WorkerId")["Work Time"].sum()
...
plot_df = pd.DataFrame(get_work_time(df))
...
Highest_grossing_films = films_2010[2]
...
print(set_index(Highest_grossing_films, "Rank"))
...
def extract_revenue(df, column, name):
    df["Revenue (millions)"] = (df["Worldwide gross"].str.replace(r"\[.*?\]", "", regex=True).apply(price_dec).astype(float) / 1e6).round(2)
    return df

top_films_revenue = extract_revenue(Highest_grossing_films, 'Worldwide gross', 'Revenue (millions')
